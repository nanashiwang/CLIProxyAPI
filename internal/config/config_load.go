...
func LoadConfigOptional(configFile string, optional bool) (*Config, error) {
	...
	if err != nil {
		if optional {
			if os.IsNotExist(err) || errors.Is(err, syscall.EISDIR) {
				...
				cfg := &Config{CredentialInFlight: DefaultCredentialInFlightConfig(), Pricing: DefaultPricingConfig(), UsageStatistics: DefaultUsageStatisticsConfig()}
	...
	if errValidate := validateCredentialWeightYAML(data); errValidate != nil {
		if optional {
			cfgOptional := &Config{CredentialInFlight: DefaultCredentialInFlightConfig(), Pricing: DefaultPricingConfig(), UsageStatistics: DefaultUsageStatisticsConfig()}
	...
	cfg.UsageStatistics = DefaultUsageStatisticsConfig()
	...
	cfg.Pricing = DefaultPricingConfig()
	...
	if errValidate := cfg.ValidateAccountPools(); errValidate != nil {
		return nil, errValidate
	}
