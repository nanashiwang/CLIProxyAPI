...
func (h *Handler) RefreshAuthFiles(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"error": coreauth.SanitizeUpstreamErrorSummary(err.Error()),
		})
		return
	}
	...
	c.JSON(http.StatusOK, gin.H{
		"ok":   true,
		"auth": gin.H{"id": refreshed.ID, "status": refreshed.Status, "disabled": refreshed.Disabled, "last_refresh": refreshed.LastRefreshedAt},
	})
}
