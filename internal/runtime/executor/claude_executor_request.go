...
// resolveClaudeFingerprintPolicy combines the selected key configuration with request authentication.
func resolveClaudeFingerprintPolicy(cfg *config.Config, auth *cliproxyauth.Auth, apiKey string) helps.ClaudeFingerprintPolicy {
	return helps.ResolveClaudeFingerprintPolicy(resolveClaudeKeyConfig(cfg, auth), auth, isClaudeOAuthToken(apiKey))
}
