...
func TestClaudeFingerprintProfileFromAuthConcurrentMetadata(t *testing.T) {
	auth := &cliproxyauth.Auth{Metadata: map[string]any{
		helps.ClaudeFingerprintProfileAttr: "claude-code-cli",
	}}
	...
	go func() {
		defer wg.Done()
		for range 1_000 {
			if got := helps.ClaudeFingerprintProfileFromAuth(auth); got != helps.ClaudeFingerprintProfileClaudeCodeCLI {
				t.Errorf("helps.ClaudeFingerprintProfileFromAuth() = %q, want %q", got, helps.ClaudeFingerprintProfileClaudeCodeCLI)
				return
			}
		}
	}()
...
func TestNormalizeClaudeFingerprintProfileWarnsOncePerValue(t *testing.T) {
	...
	defer helps.ClaudeFingerprintProfileWarned.Delete(firstTypo)
	defer helps.ClaudeFingerprintProfileWarned.Delete(secondTypo)

	for i := 0; i < 5; i++ {
		if got := helps.NormalizeClaudeFingerprintProfile(firstTypo); got != helps.ClaudeFingerprintProfileDefault {
			t.Fatalf("helps.NormalizeClaudeFingerprintProfile(%q) = %q, want default", firstTypo, got)
		}
	}
	helps.NormalizeClaudeFingerprintProfile(secondTypo)
	for i := 0; i < 3; i++ {
		helps.NormalizeClaudeFingerprintProfile("claude-code-cli")
		helps.NormalizeClaudeFingerprintProfile("")
	}
