		...
		for {
			...
			case message, ok := <-input:
				...
				case "response.steer":
					...
					metadataMu.Unlock()
					if steerQueueFull {
						if !reject("too many outstanding response.steer requests") {
							return
						}
						continue
					}
	...
	go func() {
		...
		for {
			...
			if eventType == "response.created" {
				...
				if model := gjson.GetBytes(current.clientBody, "model"); model.Exists() {
					snapshot.clientBody, _ = sjson.SetRawBytes(snapshot.clientBody, "model", []byte(model.Raw))
				}
				...
				if !firstResponse {
					...
					reporter.BeginModelObservation(current.clientBody)
			...
			reporter.ObserveResponseModelPayload(payload, "body")
