...
// Local exponential backoff stays below the provider deadline ceiling.
...
// The local implementation honors explicit sub-second provider delays.
func TestApplyAuthFailureState_SubSecondResetRetainsLocalDeadline(t *testing.T) {
	now := time.Date(2026, 9, 8, 3, 0, 0, 0, time.UTC)
	...
	if want := now.Add(tiny); !auth.Quota.NextRecoverAt.Equal(want) {
		t.Fatalf("Quota.NextRecoverAt = %v, want explicit delay %v", auth.Quota.NextRecoverAt, want)
	}
}
...
func TestQuotaCeilingPreservesNonQuotaIsolationOnLater429(t *testing.T) {
	withQuotaCooldownEnabled(t)
	for _, model := range []string{"", "model-a"} {
		t.Run("model="+model, func(t *testing.T) {
			m, a := newCooldownMonotonicManager(t, "model-a")
			m.MarkResult(context.Background(), Result{AuthID: a.ID, Model: model, Error: &Error{HTTPStatus: 404}})
			before, _ := m.GetByID(a.ID)
			next := before.NextRetryAfter
			if model != "" {
				next = before.ModelStates[model].NextRetryAfter
			}
			retry := 18 * 24 * time.Hour
			m.MarkResult(context.Background(), Result{AuthID: a.ID, Model: model, RetryAfter: &retry, Error: &Error{HTTPStatus: 429}})
			after, _ := m.GetByID(a.ID)
			got, quota := after.NextRetryAfter, after.Quota
			if model != "" {
				got, quota = after.ModelStates[model].NextRetryAfter, after.ModelStates[model].Quota
			}
			if !got.Equal(next) || quota.NextRecoverAt.After(time.Now().Add(time.Hour)) {
				t.Fatal("non-quota isolation shortened or quota unbounded", got, quota)
			}
		})
	}
}

func TestQuotaCeilingPersistedCodexEvidenceSurvivesRestart(t *testing.T) {
	m, _, a := newQuotaTestManager(t)
	now := time.Now()
	reset := now.Add(18 * 24 * time.Hour).Round(0)
	snapshot := &CodexQuotaSnapshot{Identity: codexQuotaIdentity(a), ObservedAt: now, Exhausted: true, DeniedResetAt: reset, Windows: []CodexQuotaWindow{quotaWindow("secondary", 100, reset)}}
	store := NewFileCooldownStateStore(t.TempDir())
	records := []CooldownStateRecord{
		{AuthID: a.ID, Provider: "codex", Model: "pool-model", NextRetryAfter: reset, Quota: QuotaState{Exceeded: true, Reason: "quota", NextRecoverAt: reset}},
		{AuthID: a.ID, Provider: "codex", NextRetryAfter: reset, Quota: QuotaState{Exceeded: true, Reason: "credential_quota", NextRecoverAt: reset}, CodexQuota: snapshot},
	}
	...
	if _, err := restored.Register(context.Background(), a); err != nil {
		t.Fatal(err)
	}
	...
	if got.NextRetryAfter.After(time.Now().Add(time.Hour)) || got.ModelStates["pool-model"].NextRetryAfter.After(time.Now().Add(time.Hour)) {
		t.Fatal("legacy retry persisted beyond ceiling")
	}
	if got.CodexQuota == nil || !got.CodexQuota.DeniedResetAt.Equal(reset) || !got.CodexQuota.Windows[0].ResetAt.Equal(reset) {
		t.Fatal("restart altered observed quota", got.CodexQuota)
	}
	if blocked, _, _ := isAuthBlockedForModel(got, "pool-model", now.Add(2*time.Hour)); !blocked {
		t.Fatal("ceiling released confirmed exhaustion")
	}
}

func TestQuotaCeilingCannotHideSubsequentAuthFailure(t *testing.T) {
	withQuotaCooldownEnabled(t)
	m, a := newCooldownMonotonicManager(t, "model-a", "model-b")
	retry := 18 * 24 * time.Hour
	m.MarkResult(context.Background(), Result{AuthID: a.ID, Model: "model-a", CredentialScope: true, RetryAfter: &retry, Error: &Error{HTTPStatus: 429}})
	m.MarkResult(context.Background(), Result{AuthID: a.ID, Model: "model-b", Error: &Error{HTTPStatus: 401, Message: "unauthorized"}})
	got, _ := m.GetByID(a.ID)
	future := time.Now().Add(2 * time.Hour)
	if blocked, _, _ := isAuthBlockedForModel(got, "model-a", future); !blocked {
		t.Fatal("quota expiry erased auth isolation")
	}
	if unavailable, status, _, _ := got.AvailabilityView(future); !unavailable || status != StatusError {
		t.Fatal("management concealed authentication failure")
	}
}
