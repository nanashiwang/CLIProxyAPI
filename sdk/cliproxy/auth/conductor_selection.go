	...
	if len(availableByPriority) == 0 {
		...
		if unauthorizedCount == len(auths) && len(auths) > 0 {
			...
			if terminalCause == nil {
				terminalCause = latestCandidateErrorForModel(auths, func(a *Auth) string { return m.selectionModelKeyForAuth(a, routeModel) })
			}
		...
		return nil, WithCause(&Error{Code: "auth_unavailable", Message: "no auth available"}, latestCandidateErrorForModel(auths, func(a *Auth) string { return m.selectionModelKeyForAuth(a, routeModel) }))
	}
